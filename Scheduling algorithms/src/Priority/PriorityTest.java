...
public class PriorityTest {
    ...
    private static void runTest(File testFile) throws Exception {
        ...
        Priority.Process[] processes = new Priority.Process[testData.processes.size()];
        for (int i = 0; i < testData.processes.size(); i++) {
            ProcessData p = testData.processes.get(i);
            processes[i] = new Priority.Process(p.name, p.arrival, p.burst, p.priority);
        }
        
        Priority.PriorityResult result = PriorityScheduling.executePreemptivePriority(processes, testData.contextSwitch);
        ...
        if (!result.executionOrder.equals(expected.Priority.executionOrder)) {
            System.out.println("FAILED: Execution order mismatch");
            passed = false;
        }
        
        Map<String, Priority.ProcessResult> resultMap = new HashMap<>();
        for (Priority.ProcessResult pr : result.processResults) {
            resultMap.put(pr.name, pr);
        }
        
        for (ProcessResult expectedPr : expected.Priority.processResults) {
            Priority.ProcessResult actualPr = resultMap.get(expectedPr.name);
        ...
        if (Math.abs(result.averageWaitingTime - expected.Priority.averageWaitingTime) > 0.01 ||
            Math.abs(result.averageTurnaroundTime - expected.Priority.averageTurnaroundTime) > 0.01) {
            passed = false;
        }
    ...
    private static void printResults(Priority.PriorityResult result) {
        System.out.println("\nActual Results:");
        System.out.println("Execution order: " + String.join(" -> ", result.executionOrder));
        System.out.println("\nWaiting Time for each process:");
        for (Priority.ProcessResult p : result.processResults) {
    ...
    private static TestData parseJSON(String json) {
        ...
        if (expStart != -1) {
            ...
            data.expectedOutput.Priority = new PriorityExpected();
            
            int priorityStart = json.indexOf("\"Priority\":", expStart);
            if (priorityStart != -1) {
                int execOrderKey = json.indexOf("\"executionOrder\"", priorityStart);
                if (execOrderKey != -1) {
                        ...
                        if (execOrderEnd != -1) {
                            ...
                            if (!orderStr.trim().isEmpty()) {
                                ...
                                for (String o : orders) {
                                    o = o.trim().replace("\"", "");
                                    if (!o.isEmpty()) {
                                        data.expectedOutput.Priority.executionOrder.add(o);
                                    }
                                }
                            }
                        }
                ...
                int resultsKey = json.indexOf("\"processResults\"", priorityStart);
                if (resultsKey != -1) {
                            ...
                            if (!resultsStr.trim().isEmpty()) {
                                ...
                                for (String r : results) {
                                    ...
                                    if (!r.isEmpty()) {
                                        ...
                                        data.expectedOutput.Priority.processResults.add(pr);
                                    }
                                }
                            }
                        }
                ...
                data.expectedOutput.Priority.averageWaitingTime = extractDouble(json, priorityStart, "averageWaitingTime");
                data.expectedOutput.Priority.averageTurnaroundTime = extractDouble(json, priorityStart, "averageTurnaroundTime");
            }
        }

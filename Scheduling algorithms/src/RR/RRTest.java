...
public class RRTest {
    ...
    private static void runTest(File testFile) throws Exception {
        ...
        RR.Process[] processes = new RR.Process[testData.processes.size()];
        for (int i = 0; i < testData.processes.size(); i++) {
            ProcessData p = testData.processes.get(i);
            processes[i] = new RR.Process(p.name, p.arrival, p.burst);
        }
        
        RR.RRResult result = RRScheduler.executeRoundRobin(processes, testData.rrQuantum, testData.contextSwitch);
        ...
        if (!result.executionOrder.equals(expected.RR.executionOrder)) {
            System.out.println("FAILED: Execution order mismatch");
            passed = false;
        }
        
        Map<String, RR.ProcessResult> resultMap = new HashMap<>();
        for (RR.ProcessResult pr : result.processResults) {
            resultMap.put(pr.name, pr);
        }
        
        for (ProcessResult expectedPr : expected.RR.processResults) {
            RR.ProcessResult actualPr = resultMap.get(expectedPr.name);
        ...
        if (Math.abs(result.averageWaitingTime - expected.RR.averageWaitingTime) > 0.01 ||
            Math.abs(result.averageTurnaroundTime - expected.RR.averageTurnaroundTime) > 0.01) {
            passed = false;
        }
    ...
    private static void printResults(RR.RRResult result) {
        System.out.println("\nActual Results:");
        System.out.println("Execution order: " + String.join(" -> ", result.executionOrder));
        System.out.println("\nWaiting Time for each process:");
        for (RR.ProcessResult p : result.processResults) {
    ...
    private static TestData parseJSON(String json) {
        ...
        data.rrQuantum = extractInt(json, inputStart, "rrQuantum");
        ...
        if (expStart != -1) {
            ...
            data.expectedOutput.RR = new RRExpected();
            
            int rrStart = json.indexOf("\"RR\":", expStart);
            if (rrStart != -1) {
                int execOrderKey = json.indexOf("\"executionOrder\"", rrStart);
                if (execOrderKey != -1) {
                        ...
                        if (execOrderEnd != -1) {
                            ...
                            if (!orderStr.trim().isEmpty()) {
                                ...
                                for (String o : orders) {
                                    o = o.trim().replace("\"", "");
                                    if (!o.isEmpty()) {
                                        data.expectedOutput.RR.executionOrder.add(o);
                                    }
                                }
                            }
                        }
                ...
                int resultsKey = json.indexOf("\"processResults\"", rrStart);
                if (resultsKey != -1) {
                            ...
                            if (!resultsStr.trim().isEmpty()) {
                                ...
                                for (String r : results) {
                                    ...
                                    if (!r.isEmpty()) {
                                        ...
                                        data.expectedOutput.RR.processResults.add(pr);
                                    }
                                }
                            }
                        }
                ...
                data.expectedOutput.RR.averageWaitingTime = extractDouble(json, rrStart, "averageWaitingTime");
                data.expectedOutput.RR.averageTurnaroundTime = extractDouble(json, rrStart, "averageTurnaroundTime");
            }
        }
